...
export default function FarmerDashboard() {
  ...
  const dashboard = {
    ...
    tokenStatus: activeBooking?.token_status || activeBooking?.booking_status || 'CONFIRMED',
    farmersAhead: Math.max(0, (activeBooking?.queue_position || 1) - 1),
    estimatedWaitMins: Math.max(0, (activeBooking?.queue_position || 1) - 1) * 15,
    recommendedArrival: activeBooking?.start_time || '10:30 AM',
    ...
    currentToken: '#101',
    distanceKm: '4.2',
    slotDate: activeBooking?.slot_date || new Date().toISOString().split('T')[0],
    slotTime: activeBooking?.start_time ? `${activeBooking.start_time} - ${activeBooking.end_time}` : '10:30 AM - 11:00 AM',
    slotCentre: activeBooking?.centre_name || 'Main DPC',
    slotStatus: activeBooking?.booking_status || 'Confirmed',
    procurementStage: activeBooking?.token_status === 'CALLED' ? 2 : activeBooking?.token_status === 'COMPLETED' ? 7 : 1,
