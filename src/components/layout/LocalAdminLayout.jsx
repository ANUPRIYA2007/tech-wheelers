...
import { Sprout, Home, Users as UsersIcon, Truck, UserCheck, LogOut, Moon, Sun } from 'lucide-react';

const NAV = [
  { path: '/local-admin', icon: Home, label: 'Dashboard', end: true },
  { path: '/local-admin/queue', icon: UsersIcon, label: 'Queue Management' },
  { path: '/local-admin/procurement', icon: Truck, label: 'Procurement' },
  { path: '/local-admin/farmers', icon: UserCheck, label: 'Farmers' },
];
      ...
      <aside className="hidden lg:flex lg:fixed lg:inset-y-0 lg:left-0 lg:w-64 lg:flex-col bg-white dark:bg-surface-800 border-r border-surface-200 dark:border-surface-700 z-30">
        <div className="h-16 flex items-center gap-2.5 px-6 border-b border-surface-200 dark:border-surface-700">
          ...
          <div>
            ...
            <p className="text-[10px] text-surface-400 -mt-0.5">Local Admin</p>
          </div>
        </div>
